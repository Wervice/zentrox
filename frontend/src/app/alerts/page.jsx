        ...
        res.json().then((json) => {
          ...
          if (json.temperature != "No data") {
            if (Number(json.temperature.split("°C")[0]) > 70) {
              setWarnings((prevWarnings) => {
                if (
                  !prevWarnings.includes("System temperature is above 70°C")
                ) {
                  return prevWarnings.concat(
                    "System temperature is above 70°C",
                  );
                }
                return prevWarnings;
              });
          ...
          setWarnings((prevWarnings) =>
            prevWarnings.filter(
              (warning) => warning !== "Battery is discharging",
            ),
          );
    ...
    fetch("/api/cpuPercent").then((res) => {
      if (res.ok) {
        res.json().then((json) => {
          ...
          setWarnings((prevWarnings) =>
            prevWarnings.filter(
              (warning) => warning !== "CPU Ussage is above 90%",
            ),
          );
